 ...
 * additional fields to the JSON so that we don't just get back a list of User objects.
 * 
 * For our UserRepository, the default output would like something like this for the /User :
 * 
 * {
    "_links": {
        "search": {
 ...
 * User objects, this ObjectMapper overrides the default JSON marshalling of Spring Data Rest so
 ...
 * 
 * This alternate format allows us to directly unmarshall the HTTP response bodies from the UserRepository
 * into a list of User objects.
 * 
 * @author Jules
 *
...
public class ResourcesMapper extends ObjectMapper {
	...
	// creates with the Users it obtains from your UserRepository
	...
	private JsonSerializer<Resources> serializer = new JsonSerializer<Resources>() {
		...
		public void serialize(Resources value, JsonGenerator jgen,
			...
			// that we care about (e.g., the list of User objects)
			Object content = value.getContent();
			// Instead of all of the Resources member variables, etc.
			// Just mashall the actual content (Users) into the JSON
