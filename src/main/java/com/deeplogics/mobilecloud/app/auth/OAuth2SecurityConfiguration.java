 ...
 *
 * 	The resource server is located at "/users", and can be accessed only by retrieving a token from "/oauth/token"
...
public class OAuth2SecurityConfiguration {
	...
	protected static class WebSecurityConfiguration extends WebSecurityConfigurerAdapter {
		...
		@Autowired
		private MongoUserDetailsService userRepository;
	 ...
	 *	resource server (i.e. the "/users" endpoint) 
	 */
	@Configuration
	...
	protected static class ResourceServer extends ResourceServerConfigurerAdapter {
		...
		// all of the paths in the users service.
		@Override
		public void configure(HttpSecurity http) throws Exception {
			...
			http.anonymous()
				.and()
				.authorizeRequests()
				.antMatchers("/sdoc.jsp", "/swagger-ui.js", "swagger-ui.min.js",
						"/o2c.html", "/css/**", "/fonts/**", "/images/**", "/lib/**",
						"/api-docs/**", "/health", "/docs",
						UsersSvcApi.USERS_ACTIVATEACC_PATH,
						UsersSvcApi.USERS_REGISTRATION_PATH,
						UsersSvcApi.USERS_FORGOTPASS_PATH,
						UsersSvcApi.USERS_CHANGEPASS_PATH).permitAll();
	...
	protected static class OAuth2Config extends AuthorizationServerConfigurerAdapter {
		 ...
		 * passwords listed below and updating the usersSvcClientApiTest.
		 * 
		 * @param auth
		 * @throws Exception
		 */
		public OAuth2Config() throws Exception {
			...
			ClientDetailsService csvc = new InMemoryClientDetailsServiceBuilder()
			        ...
			        // users service
					.withClient("mobile-spring").authorizedGrantTypes("password", "refresh_token")
					.accessTokenValiditySeconds(1500500500)
					.refreshTokenValiditySeconds(2000500500)
					.authorities("ROLE_CLIENT", "ROLE_TRUSTED_CLIENT")
					.scopes("read","write").resourceIds("users", "projects")
					.and()
					// Create a second client that only has "read" access to the
					// users service
					.withClient("mobileReader").authorizedGrantTypes("password", "refresh_token")
					.authorities("ROLE_CLIENT")
					.scopes("read").resourceIds("users")
					.and().build();

			 
			UserDetailsService svc = new MongoUserDetailsService();
